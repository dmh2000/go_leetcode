...
func preOrder2(node *TreeNode, s string, depth int) {
	if node != nil {
		// Visit
		fmt.Printf("%v%v:%v,", s, depth, node.Val)
		preOrder2(node.Left, "L", depth+1)
		preOrder2(node.Right, "R", depth+1)
	}
}

func inOrder2(node *TreeNode, s string, depth int) {
	if node != nil {
		inOrder2(node.Left, "L", depth+1)
		fmt.Printf("%v%v:%v,", s, depth, node.Val)
		inOrder2(node.Right, "R", depth+1)
	}
}
